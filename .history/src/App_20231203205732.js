...
const App = () => {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      fetch(url, {
        ...
        .then(response => {
          if (response.status === 200) {
            // The request was successful, you can access the response content as text
      ...
      const diagramResOut = await await window.pyodide.runPythonAsync(diagramResult, window.pyodide.toPy(
        {"result": result}));
