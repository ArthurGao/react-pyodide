...
import nolds
import requests
from io import StringIO, buffer
import base64


external_stylesheets = [
    'https://static2.sharepointonline.com/files/fabric/office-ui-fabric-core/11.0.0/css/fabric.min.css',
    'https://static2.sharepointonline.com/files/fabric/office-ui-fabric-core/11.0.0/css/fabric.components.min.css'
]
...
# Load data
df = pd.read_csv("mri_demand.csv")

exams_per_day = 20  # radiologist mean throughput
scans_per_day = 21  # MRI mean throughput

# Define the URL
url = "https://nicoforecasting.azurewebsites.net/api/forecast/070f528c-2f8a-4399-a599-cfb71faee319/746b5af1-ea1f-4081-b3f0-d48abd7313a1?code=hwDugEQ9K71-rA_RbADAsjMZwM_d9KeX1auHpm-odUj7AzFug_qugw=="
...
forecast_period = forecast[(forecast['ds'] >= forecast['ds'][len(df):].min()) & (
        forecast['ds'] <= forecast['ds'][len(df):].max())]
...
fig.add_trace(
    go.Scatter(x=[df['ds'].min(), forecast['ds'].max()], y=[100, 100],
